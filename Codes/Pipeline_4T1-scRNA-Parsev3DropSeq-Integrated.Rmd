...
# Lung differential interactions heatmap for the following: 
# Day 7 vs. Day 0
# Day 14 vs. Day 7
# Day 21 vs. Day 14
plot_list <- list()
for (i in 1:3){
  plot_list[[i]] <- netVisual_heatmap(scaf_merge_cc, measure = "weight", 
                                      sources.use = c(1:6, 10:13), targets.use = c(1:6, 10:13), 
                                      remove.isolate = T, comparison = c(i, i+1), 
                                      font.size = 13, font.size.title = 0)
}

draw(plot_list[[1]]+plot_list[[2]]+plot_list[[3]])
rm(plot_list)
```
...
# Scaffold differential interactions heatmap for the following: 
# Day 7 vs. Day 0
# Day 14 vs. Day 7
# Day 21 vs. Day 14
plot_list <- list()
...
# Plot size (WxH): 1200 x 350
cowplot::plot_grid(plot_list[[1]], plot_list[[2]], align = "h", ncol = 2)

# 
d0_temp <- plot_list[[1]]$data
d0_temp <- subset(d0_temp, subset = labels %in% c("Fibroblasts", "ECs", "Neutrophils"))
names(d0_temp)[names(d0_temp) == 'x'] <- 'outgoing'
names(d0_temp)[names(d0_temp) == 'y'] <- 'incoming'
d0_temp$Day <- "D0"

d7_temp <- plot_list[[2]]$data
d7_temp <- subset(d7_temp, subset = labels %in% c("Fibroblasts", "ECs", "Neutrophils"))
names(d7_temp)[names(d7_temp) == 'x'] <- 'outgoing'
names(d7_temp)[names(d7_temp) == 'y'] <- 'incoming'
d7_temp$Day <- "D7"

combined_df <- rbind(d0_temp, d7_temp)

# Outgoing plots
p1 <- ggplot(subset(combined_df, subset = labels == "Fibroblasts"), aes(y = outgoing, x = Day)) + 
  geom_bar(stat = "identity", position = "dodge", colour = "black", fill = "#E3BE00") + 
  theme(text = element_text(size = 20, colour = "black"),
        axis.text.x = element_text(size = 20, colour = "black"),
        axis.text.y = element_text(size = 20, colour = "black"),
  ...
  labs(y = "Outgoing Interaction Strength", title = "Fibroblasts") + ylim(0, 10)

p2 <- ggplot(subset(combined_df, subset = labels == "ECs"), aes(y = outgoing, x = Day)) + 
  geom_bar(stat = "identity", position = "dodge", colour = "black", fill = "#B2DF8A") + 
  theme(text = element_text(size = 20, colour = "black"),
        axis.text.x = element_text(size = 20, colour = "black"),
        axis.text.y = element_text(size = 20, colour = "black"),
        axis.line.x = element_line(size = 1, colour = "black", linetype=1),
        ...
        plot.background = element_rect(fill = "transparent", colour = NA_character_), 
        panel.background = element_rect(fill = "transparent", colour = NA_character_)) + 
  labs(y = "", title = "ECs") + ylim(0, 10)

cowplot::plot_grid(p1, p2, align = "h", ncol = 2)

# Incoming plots
p1 <- ggplot(subset(combined_df, subset = labels == "Fibroblasts"), aes(y = incoming, x = Day)) + 
  geom_bar(stat = "identity", position = "dodge", colour = "black", fill = "#E3BE00") + 
  theme(text = element_text(size = 20, colour = "black"),
        axis.text.x = element_text(size = 20, colour = "black"),
        axis.text.y = element_text(size = 20, colour = "black"),
        ...
        plot.background = element_rect(fill = "transparent", colour = NA_character_), 
        panel.background = element_rect(fill = "transparent", colour = NA_character_)) + 
  labs(y = "Incoming Interaction Strength", title = "Fibroblasts") + ylim(0, 2.5)

p2 <- ggplot(subset(combined_df, subset = labels == "ECs"), aes(y = incoming, x = Day)) + 
  geom_bar(stat = "identity", position = "dodge", colour = "black", fill = "#B2DF8A") + 
  theme(text = element_text(size = 20, colour = "black"),
        axis.text.x = element_text(size = 20, colour = "black"),
        axis.text.y = element_text(size = 20, colour = "black"),
        ...
        plot.background = element_rect(fill = "transparent", colour = NA_character_), 
        panel.background = element_rect(fill = "transparent", colour = NA_character_)) + 
  labs(y = "", title = "ECs") + ylim(0, 2.5)

p3 <- ggplot(subset(combined_df, subset = labels == "Neutrophils"), aes(y = incoming, x = Day)) + 
  geom_bar(stat = "identity", position = "dodge", colour = "black", fill = "#E3221C") + 
  theme(text = element_text(size = 20, colour = "black"),
        axis.text.x = element_text(size = 20, colour = "black"),
        axis.text.y = element_text(size = 20, colour = "black"),
        axis.line.x = element_line(size = 1, colour = "black", linetype=1),
        ...
        plot.background = element_rect(fill = "transparent", colour = NA_character_), 
        panel.background = element_rect(fill = "transparent", colour = NA_character_)) + 
  labs(y = "Incoming Interaction Strength", title = "Neutrophils") + ylim(0, 4)

cowplot::plot_grid(p1, p2, align = "h", ncol = 2)

rm(num.link, weight.MinMax, plot_list, i, d0_temp, d7_temp, combined_df, p1, p2, p3)
```
...
netVisual_heatmap(lung_merge_cc, measure = "weight", 
                  sources.use = c(1:6, 10:13), targets.use = c(1:6, 10:13), 
                  remove.isolate = T, comparison = c(1, 2), 
                  font.size = 13, font.size.title = 0)
...
### Figure 4D
```{r, warning=FALSE, message=FALSE}
#### Full time course fibroblast & EC signaling ####
####################################################
# Fibroblasts
# Plot size (WxH): 1200 x 500
netAnalysis_signalingChanges_scatter(lung_merge_cc, idents.use = "Fibroblasts", comparison = c(1, 2)) + 
  theme(legend.position = "none", 
        axis.text = element_text(size = 12, color = "black"), 
        axis.title.y = element_text(size = 15, color = "black"), axis.title.x = element_blank()) + 
  xlim(-0.1, 4) + ylim(-0.1, 0.7) + ggtitle(NULL) + 
  netAnalysis_signalingChanges_scatter(lung_merge_cc, idents.use = "Fibroblasts", comparison = c(2, 3)) + 
  theme(legend.position = "none", 
        axis.text = element_text(size = 12, color = "black"), 
        axis.title.x = element_text(size = 15, color = "black"), axis.title.y = element_blank()) + 
  xlim(-0.4, 0.5) + ylim(-0.1, 0.2) + ggtitle(NULL) + 
  netAnalysis_signalingChanges_scatter(lung_merge_cc, idents.use = "Fibroblasts", comparison = c(3, 4)) + 
  theme(legend.position = "none", 
        axis.text = element_text(size = 12, color = "black"), 
        axis.title = element_blank()) + 
  xlim(-3, 0.1) + ylim(-0.65, 0.1) + ggtitle(NULL)

# Endothelial Cells
# Plot size (WxH): 1200 x 430
netAnalysis_signalingChanges_scatter(lung_merge_cc, idents.use = "ECs", comparison = c(1, 2)) + 
  theme(legend.position = "none", 
        axis.text = element_text(size = 12, color = "black"), 
        axis.title.y = element_text(size = 15, color = "black"), axis.title.x = element_blank()) + 
  xlim(-0.1, 2) + ylim(-0.1, 0.3) + ggtitle(NULL) + 
  netAnalysis_signalingChanges_scatter(lung_merge_cc, idents.use = "ECs", comparison = c(2, 3)) + 
  theme(legend.position = "none", 
        axis.text = element_text(size = 12, color = "black"), 
        axis.title.x = element_text(size = 15, color = "black"), axis.title.y = element_blank()) + 
  xlim(-0.1, 0.8) + ylim(-0.1, 0.7) + ggtitle(NULL) + 
  netAnalysis_signalingChanges_scatter(lung_merge_cc, idents.use = "ECs", comparison = c(3, 4)) + 
  theme(legend.position = "none", 
        axis.text = element_text(size = 12, color = "black"), 
        axis.title = element_blank()) + 
  xlim(-1, 0.1) + ylim(-0.65, 0.1) + ggtitle(NULL)
```
...
# COLLAGEN Signaling
for (i in 1:4){
  plot_list[[i]] <- netVisual_heatmap(lungcc_object.list[[i]], signaling = "COLLAGEN", color.heatmap = "Reds",
                        title = names(lungcc_object.list)[i], font.size = 13, font.size.title = 0)
}

# Plot size (WxH): 1000 x 250
plot_list[[1]] + plot_list[[2]] + plot_list[[3]] + plot_list[[4]]

plot_list <- list()
# Fibronectin (FN1) Signaling
for (i in 1:4){
  plot_list[[i]] <- netVisual_heatmap(lungcc_object.list[[i]], signaling = "FN1", color.heatmap = "Reds",
                        title = names(lungcc_object.list)[i], font.size = 13, font.size.title = 0)
}
...
### Figure 4F
```{r, warning=FALSE, message=FALSE}
pairLR.use <- extractEnrichedLR(lung_merge_cc, signaling = c("COLLAGEN", "LAMININ", "FN1"))

# Plot size (WxH): 450 x 350
netVisual_bubble(lung_merge_cc, sources.use = c(2, 12, 13), targets.use = "Neutrophils",  comparison = c(1:4), max.dataset = 2,
                 title.name = "", angle.x = 45, remove.isolate = T, pairLR.use = pairLR.use) +
  theme(legend.position = "none", 
        axis.text.x = element_blank(), axis.text.y = element_text(size = 15, colour = "black"), 
