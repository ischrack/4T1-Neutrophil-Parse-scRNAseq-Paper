...
d0_temp <- subset(d0_temp, subset = labels %in% c("Fibroblasts", "ECs"))
names(d0_temp)[names(d0_temp) == 'x'] <- 'outgoing'
names(d0_temp)[names(d0_temp) == 'y'] <- 'incoming'
d0_temp$Day <- "D0"

d7_temp <- plot_list[[2]]$data
d7_temp <- subset(d7_temp, subset = labels %in% c("Fibroblasts", "ECs"))
...
rm(num.link, weight.MinMax, plot_list, i, d0_temp, d7_temp, combined_df, p1, p2)
```
