...
gg1 <- compareInteractions(lung_merge_cc, show.legend = F, color.use = "blue")
gg2 <- compareInteractions(lung_merge_cc, show.legend = F, measure = "weight", color.use = "blue")
gg1 + gg2
rm(gg1, gg2)
```
...
#### Figure S7A ####
object.list <- list(D0_Lung = d0_scaf_cc, D7_scaf = d7_scaf_cc, D14_scaf = d14_scaf_cc, D21_scaf = d21_scaf_cc)
merged_cc <- mergeCellChat(object.list, add.names = c("D0_scaf", "D7_scaf", "D14_scaf", "D21_scaf"))
...
netVisual_circle(object.list[[i]]@net$count, 
  ...
  netVisual_circle(object.list[[i+1]]@net$count, 
                 weight.scale = T, label.edge= F, edge.weight.max = weight.max[2], 
                 edge.width.max = 10, arrow.size = 0.05, 
                 title.name = paste0(""))

netVisual_circle(object.list[[i+2]]@net$count, 
                 weight.scale = T, label.edge= F, edge.weight.max = weight.max[2], 
                 edge.width.max = 10, arrow.size = 0.05, 
                 title.name = paste0(""))

netVisual_circle(object.list[[i+3]]@net$count, 
                 weight.scale = T, label.edge= F, edge.weight.max = weight.max[2], 
                 edge.width.max = 10, arrow.size = 0.05, 
                 title.name = paste0(""))
par(mfrow = c(1,1), xpd=TRUE)

rm(object.list, merged_cc, weight.max, i)
```
...
gg1 <- netAnalysis_signalingRole_scatter(object.list[[1]], weight.MinMax = weight.MinMax) + 
    ylim(0, 2.8) + xlim(0, 6) + 
...
gg2 <- netAnalysis_signalingRole_scatter(object.list[[2]], weight.MinMax = weight.MinMax) + 
    ylim(0, 2.8) + xlim(0, 6) + 
    theme(axis.line = element_line(color = "black", size = 2), 
          axis.text = element_text(color = "black", size = 12),
...
gg3 <- netAnalysis_signalingRole_scatter(object.list[[3]], weight.MinMax = weight.MinMax) + 
    ylim(0, 2.8) + xlim(0, 6) + 
    theme(axis.line = element_line(color = "black", size = 2), 
          axis.text = element_text(color = "black", size = 12),
          axis.title.y = element_blank(), 
...
gg4 <- netAnalysis_signalingRole_scatter(object.list[[4]], weight.MinMax = weight.MinMax) + 
    ylim(0, 2.8) + xlim(0, 6) + 
    theme(axis.line = element_line(color = "black", size = 2), 
          axis.text = element_text(color = "black", size = 12),
          axis.title.y = element_blank(), 
...
cowplot::plot_grid(gg1, gg2, gg3, gg4, align = "h", ncol = 4)

rm(object.list, merged_cc, weight.max, i, gg1, gg2, gg3, gg4, num.link, weight.MinMax)
```
